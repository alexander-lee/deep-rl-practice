...
class QLearner(object):
    ...
    def step_env(self):
        ...
        # Added: Environment stepping and storing the experience in the replay buffer
        self.replay_buffer_idx = self.replay_buffer.store_frame(self.last_obs)

        # Always explore (random action) if not initialized, otherwise grab epsilon value
        exploration_prob = self.exploration.value(self.t)
        explore = not self.model_initialized or np.random.rand() < exploration_prob
        ...
        # DEBUG: Show results of every N episodes
        ...
        if done:
            ...
            self.episode += 1
            if self.episode % 20 == 0:
                print('VIEWING EPISODE {}'.format(self.episode))
    ...
    def update_model(self):
        ...
        if (self.t > self.learning_starts and self.t % self.learning_freq == 0 and self.replay_buffer.can_sample(self.batch_size)):
            ...
            # Added model training and updating
