...
class Agent(object):
    ...
    def define_placeholders(self):
        """
            ...
            returns:
                ...
                sy_adv_n: placeholder for advantages (rewards (- baseline))
        """
        ...
        """ Constructs the symbolic operation for the policy network outputs,
            ...
            returns:
                ...
                if continuous, the parameters are a tuple (mean, log_std) of a Gaussian
                    ...
                    sy_logstd: (self.ac_dim,) (Just ac_dim because its the std for all batches)
    ...
    def build_computation_graph(self):
        ...
        # ========================================================================================#
        # Added: loss function (log probability * rewards)
    ...
    def sum_of_rewards(self, re_n):
        ...
        # r_n = [
        #     [reward at time = 1 for trajectory = 0, reward at time = 2 for trajectory = 1, ...],
        #     [reward at time = 1 for trajectory = 1, reward at time = 2 for trajectory = 2, ...]
        # ]

        if self.reward_to_go:
            # Added: reward-to-go policy gradient
            q_n = []
            for trajectory in re_n:
                # If input is [1,2,3]
                # output is [1 * gamma^0 + 2 * gamma^1 + 3 * gamma^2, 2 * gamma^1 + 3 * gamma^2, 3 * gamma^2]
                # So we compute the discounted rewards backwards and use a cumulative sum
            ...
            q_n = []
            for trajectory in re_n:
                discounted_rewards = [(self.gamma ** t) * re for (t, re) in enumerate(trajectory, 0)]
                q_t = np.repeat(np.sum(discounted_rewards), len(trajectory))
                q_n = np.concatenate((q_n, q_t))
